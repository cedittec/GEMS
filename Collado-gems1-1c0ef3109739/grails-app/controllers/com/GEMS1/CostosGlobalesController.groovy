...
class CostosGlobalesController {
def springSecurityService
   def index() { 
        ...
        def costoEnergiaElectrica  = 0
        def costoEnergiaTermica = 0
        def costoCombustibles = 0
        ...
        num = sitios.size()
          ...
          costoEnergiaElectrica  += auxiliarDC027(sitios.get(0),fechaHoy, fechaMesAtras)
          costoEnergiaTermica += auxiliarDC048(sitios.get(0),fechaHoy, fechaMesAtras)
          costoCombustibles += auxiliarDC061(sitios.get(0),fechaHoy, fechaMesAtras)
          
        
          if(costoEnergiaElectrica <0)
            costoEnergiaElectrica = 0.0
            if(costoEnergiaTermica<0)
            costoEnergiaTermica=0.0
            if(costoCombustibles<0)
            costoCombustibles = 0.0
       ...
       [sitios:sitios, numeroSitios:num, rol:rol, costoEnergiaElectrica:costoEnergiaElectrica, costoEnergiaTermica:costoEnergiaTermica,costoCombustibles:costoCombustibles 
       ,arregloGraficaValores3:arregloGraficaValores3,arregloGraficaMeses:arregloGraficaMeses,arregloGraficaValores:arregloGraficaValores,
        arregloGraficaValores2:arregloGraficaValores2]
        
    }
    
    def costoEnergiaElectrica()
    {
        def num =0
        def sitios
        def principal = springSecurityService.principal
        ...
        def arregloEnergiaTotal = [0.0,0.0,0.0,0.0]
        def arregloEnergiaTotalCFE =[0.0,0.0,0.0,0.0]
        def arregloCostoEvitadoCogeneracion = [0.0,0.0,0.0,0.0]
        def arregloCostoEvitadoFotovoltaico = [0.0,0.0,0.0,0.0]
        def tipoTarifa ="-"
        ...
        num = sitios.size()
          if(num>0){
            ...
            //Arreglo que devuelve [0]:Energia total, [1]:Energia base total, [2]:energia inter total, [3]:energia punta total
            //Para primer cuadro de la pantalla
            arregloEnergiaTotal = reporteC001(sitios.get(0),fechaHoy, fechaMesAtras)
            arregloEnergiaTotalCFE = reporteC001EnergiaTotalCFE(sitios.get(0),fechaHoy, fechaMesAtras)
            arregloCostoEvitadoCogeneracion = reporteC001CostoEvitadoCogeneracion(sitios.get(0),fechaHoy, fechaMesAtras)
            arregloCostoEvitadoFotovoltaico = reporteC001CostoEvitadoFotovoltaico(sitios.get(0),fechaHoy, fechaMesAtras)
            tipoTarifa = tipoTarifaSitio(sitios.get(0))
            
            
            def arregloResultadoGrafica = graficaCostoEnergiaElectrica(sitios.get(0))
       ...
       [sitios:sitios, numeroSitios:num, rol:rol,arregloEnergiaTotal:arregloEnergiaTotal,arregloEnergiaTotalCFE:arregloEnergiaTotalCFE,
       arregloCostoEvitadoCogeneracion:arregloCostoEvitadoCogeneracion,arregloCostoEvitadoFotovoltaico:arregloCostoEvitadoFotovoltaico,
       tipoTarifa:tipoTarifa,arregloGraficaMeses:arregloGraficaMeses,arregloGraficaValores:arregloGraficaValores,arregloGraficaValores2:arregloGraficaValores2,
      arregloGraficaValores3:arregloGraficaValores3,arregloGraficaValores4:arregloGraficaValores4]                
    }
    
    def costoEnergiaTermica()
    {
        def num =0
        def sitios
        def principal = springSecurityService.principal
        ...
        def arregloEficienciaTermica = [0.0,0.0,0.0]
        def arregloCostoEvitadoAlternas = [0.0,0.0,0.0,0.0]
        def arregloPrecioUnitarioCombustibles = [0.0,0.0]
        def arregloKWHEnergiaTermica = [0.0,0.0,0.0]
        ...
        num = sitios.size()
          if(num>0){
            ...
            arregloEficienciaTermica = eficienciaTermicaReporteC003(sitios.get(0), fechaHoy, fechaMesAtras)
            arregloCostoEvitadoAlternas = costoEvitadoFuentesAlternasReporteC003(sitios.get(0), fechaHoy, fechaMesAtras)
            
            //Si devuelve Ceros, en la Vista se debe mtrar que se toman de las páginas correspondientes
            arregloPrecioUnitarioCombustibles =  precioUnitarioCombustibles(sitios.get(0), fechaHoy, fechaMesAtras)
            
            arregloKWHEnergiaTermica =   costoKWHEnergiaTermica(sitios.get(0), fechaHoy, fechaMesAtras)
            
            //GRAFICA DATOS
            def arregloResultadoGrafica = graficaCostoEnergiaTermica(sitios.get(0))
       ...
       [sitios:sitios, numeroSitios:num, rol:rol, arregloEficienciaTermica:arregloEficienciaTermica,arregloCostoEvitadoAlternas:arregloCostoEvitadoAlternas,
       arregloPrecioUnitarioCombustibles:arregloPrecioUnitarioCombustibles,arregloKWHEnergiaTermica:arregloKWHEnergiaTermica,
    ...
    def costoCombustibles()
    {
        ...
        def rol = UsuarioRol.findByUsuario(usuario)
        rol = Rol.findById(rol.rol.id)   
        def arregloLecturaInicialFinalCogeneracion = [0.0,0.0]
        def arregloLecturaInicialFinalTermico = [0.0,0.0]
        def arregloCostoCombustibles= [0.0,0.0,0.0]
        def arregloPrecioUnitarioCombustibles = [0.0,0.0]
        def tipoCombustibleCog = ""
        def tipoCombustibleTermico = ""
        ...
        if(num>0){
            ...
            arregloLecturaInicialFinalCogeneracion = lecturaInicialFinalCogeneracion(sitios.get(0), fechaHoy, fechaMesAtras)
            
            arregloLecturaInicialFinalTermico = lecturaInicialFinalTermico(sitios.get(0), fechaHoy, fechaMesAtras) 
            arregloCostoCombustibles =  costosDeCombustibles(sitios.get(0), fechaHoy, fechaMesAtras) 
            ...
            if(fuentes){
                ...
                tipoCombustibleTermico = tipoCombustible(fuentes[0])
                arregloPrecioUnitarioCombustibles =  precioUnitarioCombustibles(sitios.get(0), fechaHoy, fechaMesAtras)
                println "PRECIO UNI : "+arregloPrecioUnitarioCombustibles
                //Grafica:
                def arregloResultadoGrafica = graficaCostoCombustibles(sitios.get(0))
        ...
        [sitios:sitios, numeroSitios:num, rol:rol,arregloLecturaInicialFinalCogeneracion:arregloLecturaInicialFinalCogeneracion,
        arregloLecturaInicialFinalTermico:arregloLecturaInicialFinalTermico,arregloCostoCombustibles:arregloCostoCombustibles,
        tipoCombustibleCog:tipoCombustibleCog,tipoCombustibleTermico:tipoCombustibleTermico,arregloPrecioUnitarioCombustibles:arregloPrecioUnitarioCombustibles,
        arregloGraficaMeses:arregloGraficaMeses,arregloGraficaValores:arregloGraficaValores,arregloGraficaValores2:arregloGraficaValores2,arregloGraficaValores3:arregloGraficaValores3]    
    }
   
 //***************************METODOS DE CALCULOS*****************************************
 
     def graficaCostoEnergiaTermica(sitio)
    {
         ...
         use (TimeCategory) {
                ...
                while(i<12)
                {
                    ...
                    if((i+1) == 12)
                    {
                        ...
                        //println "QUEDA DE "+fechaAtras+" a "+fechaTempActual
                    }
                    def arregloEnergiaAprovechadaCog =  energiaAprovechadaCogeneracion(sitio, fechaTempActual, fechaAtras)         
                    def DT062 = arregloEnergiaAprovechadaCog[0]+ arregloEnergiaAprovechadaCog[1] + arregloEnergiaAprovechadaCog[2]
                    def DC044 = auxiliarDC044(sitio, fechaTempActual, fechaAtras)  
                    def DC046 = (DT062 * DC044)
                                  
                
                    def DT063 =auxiliarDT063(sitio, fechaTempActual, fechaAtras)
                    def DC045 = auxiliarDC045(sitio, fechaTempActual, fechaAtras)   
                    def DC047 = (DT063*DC045)
                                                        
                    def DC0491 = 0 //??
                    def DC0492 = DC045 - DC0491
                
                    valoresXmes1[i] = (double) DC046
                    
                    valoresXmes2[i] = (double) DC047
                    valoresXmes3[i] = (double) DC0492
    ...
    def graficaCostoEnergiaElectrica(sitio)
    {
        Date fechaHoy = new Date();            
        def fechaAtras
        def valoresXmes1=[12] //El primer arreglo para el primer conjunto de valores a graficar (este caso ahorro energias alternas)
        ...
        def i = 0;
         ...
         use (TimeCategory) {
                fechaAtras = fechaHoy - 11.month
                fechaAtras = new Date(fechaAtras.year, fechaAtras.month, 1)
//                println "mes de fecha atras: "+fechaAtras
//                println "mes de fecha actual: "+ fechaHoy
                ...
                while(i<12)
                {
                    ...
                    def arreglo1 = reporteC001(sitio, fechaTempActual, fechaAtras)                   
                    valoresXmes1[i] = (double) arreglo1[0]
                    valoresXmes2[i] = (double) arreglo1[1]
                    valoresXmes3[i] = (double) arreglo1[2]
                    valoresXmes4[i] = (double) arreglo1[3]
        ...
        def i = 0;
         ...
         use (TimeCategory) {
                ...
                while(i<12)
                {
                    ...
                    valoresXmes3[i] = (double) auxiliarDC061(sitio, fechaTempActual, fechaAtras)
                    Calendar cal = Calendar.getInstance();
                    cal.setTime(fechaTempActual);  
                    SimpleDateFormat format1 = new SimpleDateFormat("yyyy-MM-dd");                    
                    String formatted = format1.format(cal.getTime());
  ...
  def graficaCostoCombustibles(sitio)
    {
        Date fechaHoy = new Date();            
        def fechaAtras
        def valoresXmes1=[12] //El primer arreglo para el primer conjunto de valores a graficar (este caso ahorro energias alternas)
        ...
        def i = 0;
         ...
         use (TimeCategory) {
                ...
                while(i<12)
                {
                    ...
                    def arreglo = costosDeCombustibles(sitio, fechaTempActual, fechaAtras)
                
                    valoresXmes1[i] = (double) arreglo[0]
                    valoresXmes2[i] = (double) arreglo[1]
                    valoresXmes3[i] = (double) arreglo[2]
                    Calendar cal = Calendar.getInstance();
                    cal.setTime(fechaTempActual);  
                    SimpleDateFormat format1 = new SimpleDateFormat("yyyy-MM-dd");                    
                    String formatted = format1.format(cal.getTime());
   ...
   def actualizaReporteCostoCombustibles(){
        ...
        arregloLecturaInicialFinalCogeneracion = lecturaInicialFinalCogeneracion(thisSite,fechaEnd, fechaInit)
        arregloLecturaInicialFinalTermico = lecturaInicialFinalTermico(thisSite,fechaEnd, fechaInit)
        arregloCostoCombustibles =  costosDeCombustibles(thisSite,fechaEnd, fechaInit)
        ...
        if(fuentes){
            ...
            for(int i=0; i<arregloLecturaInicialFinalCogeneracion.size(); i++)
            {
                arregloLecturaInicialFinalCogeneracion[i] = (double)arregloLecturaInicialFinalCogeneracion[i]
            }
             for(int i=0; i<arregloLecturaInicialFinalTermico.size(); i++)
            {
                arregloLecturaInicialFinalTermico[i] = (double)arregloLecturaInicialFinalTermico[i]
            }
             for(int i=0; i<arregloCostoCombustibles.size(); i++)
            {
                arregloCostoCombustibles[i] = (double)arregloCostoCombustibles[i]
            }
            
        tipoCombustibleCog = tipoCombustible(fuentes[0])
        tipoCombustibleTermico = tipoCombustible(fuentes[0])
            arregloPrecioUnitarioCombustibles =  precioUnitarioCombustibles(thisSite,fechaEnd, fechaInit)
             for(int i=0; i<arregloPrecioUnitarioCombustibles.size(); i++)
            {
                arregloPrecioUnitarioCombustibles[i] = (double)arregloPrecioUnitarioCombustibles[i]
            }
            //Grafica:
            def arregloResultadoGrafica = graficaCostoCombustibles(thisSite)
        ...
        def resultado = []
        resultado.add(arregloLecturaInicialFinalCogeneracion[0])
        resultado.add(arregloLecturaInicialFinalCogeneracion[1])
        resultado.add(arregloLecturaInicialFinalTermico[0])
        resultado.add(arregloLecturaInicialFinalTermico[1])
        
        resultado.add(arregloCostoCombustibles[0])
        resultado.add(arregloCostoCombustibles[1])
        resultado.add(arregloCostoCombustibles[2])
        
        resultado.add(tipoCombustibleCog)
        resultado.add(tipoCombustibleTermico)
        
        resultado.add(arregloPrecioUnitarioCombustibles)
    ...
    def actualizaReporteEnergiaElectrica(){
        ...
        arregloEnergiaTotal = reporteC001(thisSite, fechaEnd, fechaInit)
        
        arregloEnergiaTotalCFE = reporteC001EnergiaTotalCFE(thisSite, fechaEnd, fechaInit)
        arregloCostoEvitadoCogeneracion = reporteC001CostoEvitadoCogeneracion(thisSite, fechaEnd, fechaInit)
        arregloCostoEvitadoFotovoltaico = reporteC001CostoEvitadoFotovoltaico(thisSite, fechaEnd, fechaInit)
        tipoTarifa = tipoTarifaSitio(thisSite)
                
        for(int i=0; i<arregloEnergiaTotal.size(); i++)
        {
            arregloEnergiaTotal[i] = (double) arregloEnergiaTotal[i]
        }
        for(int i=0; i<arregloEnergiaTotalCFE.size(); i++)
        {
            arregloEnergiaTotalCFE[i] = (double) arregloEnergiaTotalCFE[i]
        }
        for(int i=0; i<arregloCostoEvitadoCogeneracion.size(); i++)
        {
            arregloCostoEvitadoCogeneracion[i] = (double) arregloCostoEvitadoCogeneracion[i]
        }
        for(int i=0; i<arregloCostoEvitadoFotovoltaico.size(); i++)
        {
            arregloCostoEvitadoFotovoltaico[i] = (double) arregloCostoEvitadoFotovoltaico[i]
        }
        
        def arregloResultadoGrafica = graficaCostoEnergiaElectrica(thisSite)
        ...
        def resultado = []
         ...
         println "tipo tarifa: " + tipoTarifa
        resultado.add(tipoTarifa)
        
        resultado.add(arregloEnergiaTotal[0])
        resultado.add(arregloEnergiaTotal[1])
        resultado.add(arregloEnergiaTotal[2])
        resultado.add(arregloEnergiaTotal[3])
        ...
        resultado.add(arregloCostoEvitadoCogeneracion[0])
        resultado.add(arregloCostoEvitadoCogeneracion[1])
        resultado.add(arregloCostoEvitadoCogeneracion[2])
        resultado.add(arregloCostoEvitadoCogeneracion[3])
        
        resultado.add(arregloCostoEvitadoFotovoltaico[0])
        resultado.add(arregloCostoEvitadoFotovoltaico[1])
        resultado.add(arregloCostoEvitadoFotovoltaico[2])
        resultado.add(arregloCostoEvitadoFotovoltaico[3])
    ...
    def actualizaReporteEnergiaTermica(){
        ...
        arregloEficienciaTermica = eficienciaTermicaReporteC003(thisSite, fechaEnd, fechaInit)
        arregloCostoEvitadoAlternas =  costoEvitadoFuentesAlternasReporteC003(thisSite, fechaEnd, fechaInit)
         
        //Si devuelve Ceros, en la Vista se debe mtrar que se toman de las páginas correspondientes
        arregloPrecioUnitarioCombustibles = precioUnitarioCombustibles(thisSite, fechaEnd, fechaInit)
         
        arregloKWHEnergiaTermica = costoKWHEnergiaTermica(thisSite, fechaEnd, fechaInit)
                
        for(int i=0; i<arregloEficienciaTermica.size(); i++)
        {
            arregloEficienciaTermica[i] = (double) arregloEficienciaTermica[i]
        }
        for(int i=0; i<arregloCostoEvitadoAlternas.size(); i++)
        {
            arregloCostoEvitadoAlternas[i] = (double) arregloCostoEvitadoAlternas[i]
        }
        for(int i=0; i<arregloPrecioUnitarioCombustibles.size(); i++)
        {
            arregloPrecioUnitarioCombustibles[i] = (double) arregloPrecioUnitarioCombustibles[i]
        }
        for(int i=0; i<arregloKWHEnergiaTermica.size(); i++)
        {
            arregloKWHEnergiaTermica[i] = (double) arregloKWHEnergiaTermica[i]
        }
        
        //GRAFICA DATOS
        def arregloResultadoGrafica = graficaCostoEnergiaTermica(thisSite)
        ...
        def resultado = []
        resultado.add(arregloEficienciaTermica)
        resultado.add(arregloCostoEvitadoAlternas)
        resultado.add(arregloPrecioUnitarioCombustibles)
        resultado.add(arregloKWHEnergiaTermica)
    ...
    def actualizaReporteIndex(){
        ...
        def costoEnergiaElectrica = 0.0
        def costoEnergiaTermica = 0.0
        def costoCombustibles = 0.0
        ...
        def thisSite = Sitio.findById(Integer.parseInt(params?.sitioId))
         ...
         costoEnergiaTermica += (double)auxiliarDC048(thisSite,fechaEnd, fechaInit)
         costoCombustibles += (double)auxiliarDC061(thisSite,fechaEnd, fechaInit)
        ...
        def resultado = []
        resultado.add(costoEnergiaElectrica)
        resultado.add(costoEnergiaTermica)
        resultado.add(costoCombustibles)
    ...
    def lecturaInicialFinalCogeneracion(sitio,fechaActual, fechaMesAtras)
    {
        ...
        if(fuentes)
        {        
            def fuente = fuentes[0]
            def tuplas = RegistroSistemaCogeneracionHora.findAll {
                    dateCreated <= (fechaActual) && (dateCreated >=fechaMesAtras) && (fuente)
                }
            
            def tam = tuplas.size()-1;
            arregloResult[0] = (float) tuplas[0].fuelEnergyFlow
            arregloResult[1] = (float) tuplas[tam].fuelEnergyFlow      
    ...
    def lecturaInicialFinalTermico(sitio,fechaActual, fechaMesAtras)
    {
        ...
        {        
            ...
            if(tuplas){
            def tam = tuplas.size()-1;       
            arregloResult[0] += (float) tuplas[0].flujoCombustible
            arregloResult[1] += (float) tuplas[tam].flujoCombustible         
            }
        }
    ...
    //Devuelve un arreglo de 4 conteniendo lo necesario para el reporte C003, y para desplegar en
    //pantalla el cual es: [0]:total, [1]:Costo evitado cog. , [2]: costo evitado Termico, [3]:Costo evistado Colector
    ...
    def reporteC001EnergiaTotalCFE(sitio, fechaActual, fechaMesAtras)
    {
        ...
        //println "SE devuelve CFE: "+arregloRestult[0]+"+"+arregloRestult[1]+"+"+arregloRestult[2]+"+"+arregloRestult[3]+";"
    ...
    def energiasBIPCogeneracion(sitio, fechaActual, fechaMesAtras)
    {
        ...
        def fuenteMicroturbina = FuenteDeEnergia.findByTipo("Microturbina")                  
        def arregloEnergias = [0.0,0.0,0.0]
        def fuentes = FuenteSitio.findAllBySitioAndFuenteDeEnergiaId(sitio,fuenteMicroturbina.id)
        def horasOperacion = 0
