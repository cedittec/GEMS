        ...
        //hacer llamada a método para actualizar
            ...
            xmlhttp.onreadystatechange=function()
            {
                if (xmlhttp.readyState==4 && xmlhttp.status==200)
                {
                    ...
                    $dataXml = $( xmlDoc );
                    algo = $dataXml.find('array')[1];
                    var cogElectrica = $dataXml.find('double');
                    console.log(cogElectrica);
                    $("#reduccionEmisionesGE").html(cogElectrica[9].innerHTML + " lbs CO2");
                    $("#reduccionEmisionesDiesel").html(algo.childNodes[0].innerHTML + " lbs CO2");
                    $("#reduccionDieselTermica").html(algo.childNodes[1].innerHTML + " lbs CO2");
                    $("#reduccionDieselCogeneracion").html(algo.childNodes[2].innerHTML + " lbs CO2");
                    
                     var meses= $dataXml.find('string');  
                    actualizarGrafica(cogElectrica,meses);
            ...
            xmlhttp.open("GET","actualizaEmisionesCogeneracion?sitioId="+sitioId+"&fechaInicio="+fechaInicio+"&fechaFinal="+fechaFinal,true);
            xmlhttp.send();
    }
...
Morris.Area({
    ...
    $("#comenzarPDF").click(function(){
        ...
        txt = $("#reduccionDieselTermica").html();           
        $("#idtermica").val( txt.trim() );           
         
         txt = $("#reduccionDieselCogeneracion").html();
        $("#idcogeneracion").val( txt.trim() );    
        
        txt = $("#reduccionEmisionesDiesel").html();
        $("#idemisionesCombustible").val( txt.trim() );
        
         txt = $("#reduccionEmisionesGE").html();
        $("#idemsionesElectrica").val( txt.trim() );        
